...
# Exceeding the `uint96` Limit due to the certain ERC20 tokens like UNI or COMP use uint96 for balance and transfer operations, but ZeroLend does not integrate these tokens so this will be considered as out-of-scope, and is Not a Concern due to the following reason

- ZeroLend  is designed to only integrate with standard ERC20 tokens, USDC, and BNB. 
however in the `executeWithdraw` function, the `params.amount` used  `safeTransfer`  to validated  any transfer before it occurs [here](https://github.com/sherlock-audit/2024-06-new-scope/blob/c8300e73f4d751796daad3dadbae4d11072b3d79/zerolend-one/contracts/core/pool/logic/SupplyLogic.sol#L146) 
