...
### 1. This will be consider as out-of-scope
In zeroLend only standard ERC20 tokens + USDC and BNB are in-scope and those tokens operate with balances and supply values that are far below the `uint96` limit, which can hold values up to `2^96 - 1` `the concern about exceeding the `uint96` limit on ERC20 tokens that use `uint96` for balance and transfer operations like UNI or COMP does not apply in this context.`
- Heres why: 
The `validateWithdraw` function ensures any amount being withdrawn is valid before any action 

https://github.com/sherlock-audit/2024-06-new-scope/blob/c8300e73f4d751796daad3dadbae4d11072b3d79/zerolend-one/contracts/core/pool/logic/ValidationLogic.sol#L96
```solidity
...
1. **`require(amount != 0)`**: Prevents zero-amount withdrawals.
2. **`require(amount <= userBalance)`**: Ensures the withdrawal amount is less than or equal to the user's balance, this prevent the excessive withdrawals.

### Why This Solves the `uint96` Limit Concern:
- The withdrawal amount is always within the user's balance, far below the `uint96` limit.
- The transfer is performed using `safeTransfer` (which uses `uint256`), there is no risk of exceeding the `uint96` limit.
- Even if the balance were stored using `uint96`, the validation would ensure that no invalid transfers occur.

Thus, the combination of `validateWithdraw` and `safeTransfer` eliminates any concerns about exceeding the `uint96` limit 
The `validateWithdraw` function acts as a guard, to ensure that only valid amounts are transferred.
